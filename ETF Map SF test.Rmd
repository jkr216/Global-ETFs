---
title: "ETF Map DataGrab SF Test"
output:
...
sf_world <- ne_countries(type = "countries", returnclass='sf')
test<- st_simplify(sf_world, preserveTopology = FALSE, dTolerance = 0.0)
...
world@data[c('name', 'gdp_md_est', 'economy')]
sf_world[c('name', 'gdp_md_est', 'economy')]
```
...
gdpPal <- colorQuantile("Blues", sf_world$gdp_md_est, n = 20)
```
...
leaf_world_economy <- leaflet(sf_world) %>%
...
world_etf <- merge(sf_world, etf_ticker_country, by = "name")
```
